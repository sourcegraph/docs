...
> See the [metrics and dashboards documentation](/admin/observability/metrics#prometheus).
...
Adding recording rules, alerts, etc. to this image is handled by the [monitoring generator](/dev/background-information/observability/monitoring-generator).
...
See the [metrics and dashboards documentation](/admin/observability/metrics#grafana).

To learn more about developing metrics, see the [observability developer guides](/dev/background-information/observability/#guides).
...
To learn more about developing our observability stack, see the [local Sourcegraph monitoring development guide](/dev/how-to/monitoring_local_dev).
...
1. [Run the monitoring stack locally](/dev/how-to/monitoring_local_dev) and verify that:
   1. If upgrading Prometheus: all Prometheus rules are evaluated successfully (`localhost:9090/rules`)
   1. If upgrading Alertmanager: Alertmanager starts up correctly (`localhost:9090/alertmanager/#/status`), and [`observability.alerts` can be configured in site config](/admin/observability/alerting) (check this by adding an entry, e.g. Slack alerts) via the Sourcegraph web application, e.g:
