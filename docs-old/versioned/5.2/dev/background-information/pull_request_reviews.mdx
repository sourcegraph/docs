...
> NOTE: Supported with SOC2/GN-98 and SOC2/GN-104
...
In addition to being peer-reviewed, all contributions must pass our [continuous integration](/dev/background-information/ci/).
...
- Writing a good PR description (and [commit messages](/dev/background-information/commit_messages)) develops your technical communication skills.
...
- Prefer small and incremental PRs. If a large PR is unavoidable, try to organize it into smaller commits (with [good commit messages](/dev/background-information/commit_messages)) that are reviewable independently (and indicate this to the reviewer in the description).
...
Special care should be taken when reviewing a diff that contains, or is adjacent to, comments that contain the following string: `🚨 SECURITY`. These comments indicate security-sensitive code, the correctness of which is necessary to ensure that no private data is accessed by unauthorized actors. The code owner of any modified security-sensitive code must approve the changeset before it is merged. Please refer to [Security patterns](/dev/background-information/security_patterns) for considerations when touching security-sensitive code.
