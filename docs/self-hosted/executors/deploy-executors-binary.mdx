...
See [Executor configuration](/self-hosted/executors/executors-config) for a full list of configuration options for the executor service, and [Queues](/self-hosted/executors/executors-config#queues) for which queues each feature requires.

```bash
# Example:
export EXECUTOR_FRONTEND_URL=http://sourcegraph.example.com
export EXECUTOR_FRONTEND_PASSWORD=SUPER_SECRET_SHARED_TOKEN
export EXECUTOR_QUEUE_NAMES=batches,batches-hooks,codeintel
```
...
EXECUTOR_FRONTEND_URL="https://sourcegraph.example.com"
