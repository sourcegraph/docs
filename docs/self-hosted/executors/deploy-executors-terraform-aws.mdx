...
You can connect to the instance using [AWS Systems Manager Session Manager](https://docs.aws.amazon.com/systems-manager/latest/userguide/session-manager.html) (if the instance has the SSM agent and appropriate IAM role) or via SSH.

```shell
# Using SSM (no SSH key required)
aws ssm start-session --target i-0abc123def456789

# Using SSH (requires the key pair configured in the module)
