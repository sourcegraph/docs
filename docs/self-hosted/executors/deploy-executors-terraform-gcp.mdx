...
module "executors" {
  ...
  # Either a single queue:
  executor_queue_name                          = "codeintel"
  # Or multiple queues (batch changes need both batches and batches-hooks):
  executor_queue_names                         = ["batches", "batches-hooks", "codeintel"]
...
| `executor_queue_name`                          | The single [queue](/self-hosted/executors/executors-config#queues) from which the executor should pull jobs. Either this or `executor_queue_names` must be set.                                                                   |
| `executor_queue_names`                         | The [queues](/self-hosted/executors/executors-config#queues) from which the executor should pull jobs. Executors serving batch changes need both `batches` and `batches-hooks`. Either this or `executor_queue_name` must be set. |
