...
If Sourcegraph's builtin [alerting](/self-hosted/observability/alerting) (which can notify you via email, Slack, PagerDuty, webhook, and more) is not sufficient for you, or if you just prefer to consume the alerts programmatically for some reason, then this page is for you.

For more information about Sourcegraph alerts, see [high level alerting metrics](/self-hosted/observability/metrics#high-level-alerting-metrics).
...
```sh
curl 'http://prometheus.example.com/api/v1/query?query=sum%28max%20by%20%28service_name%2Cname%2Cdescription%29%28max_over_time%28alert_count%7Blevel%3D%22critical%22%2Cname%21%3D%22%22%7D%5B1m%5D%29%29%29
```
...
```sh
curl 'http://prometheus.example.com/api/v1/query?query=sum%20by%20%28service_name%29%28max%20by%20%28service_name%2Cname%2Cdescription%29%28max_over_time%28alert_count%7Blevel%3D%22critical%22%2Cname%21%3D%22%22%7D%5B1m%5D%29%29%29'
```
...
```sh
curl 'http://prometheus.example.com/api/v1/query?query=max%20by%20%28service_name%2Cname%2Cdescription%29%28max_over_time%28alert_count%7Blevel%3D%22critical%22%2Cname%21%3D%22%22%7D%5B1m%5D%29%29'
```
