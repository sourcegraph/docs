...
Note: For request logs, see [Outbound request log](/admin/outbound-request-log).
...
-   `logfmt`: The [logfmt](https://github.com/kr/logfmt) format.
    -   Note that `logfmt` is no longer supported with Sourcegraph's new internal logging standards - if you need structured logs, we recommend using `json` instead. If set to `logfmt`, log output from new loggers will be in `condensed` format.
...
This is configured by the environment variable `SRC_LOG_SCOPE_LEVEL`. It has the format `SCOPE_0=LEVEL_0,SCOPE_1=LEVEL_1,...`. For example to turn on debug logs for `service.UpdateScheduler` and `repoPurgeWorker` you would set the following on the `worker` service:
