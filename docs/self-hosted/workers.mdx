...
This job periodically checks for repositories that can be auto-indexed and queues indexing jobs for a remote executor instance to perform. Read how to [enable](/code-navigation/auto_indexing#enable-auto-indexing) and [configure](/code-navigation/auto_indexing#configure-auto-indexing) auto-indexing.
...
This job periodically checks for dependency packages that can be auto-indexed and queues indexing jobs for a remote executor instance to perform. Read how to [enable](/code-navigation/auto_indexing#enable-auto-indexing) and [configure](/code-navigation/auto_indexing#configure-auto-indexing) auto-indexing.
...
This job bulk encrypts existing data in the database when an encryption key is introduced, and decrypts it when instructed to do. See [encryption](/self-hosted/encryption) for additional details.
...
**Scaling notes**: There must always be just a _single instance_ of `license-check` worker. Scaling this worker horizontally would result in unexpected behavior. See [the horizontal scaling second](#2-scale-horizontally) below for additional details.
