...
_To get a better idea of your resource requirements for your instance use our [resource estimator](/self-hosted/deploy/resource-estimator)._

## Deployment Best Practices

A comparison table of supported self-hosted deployment methodologies can be [found here](/self-hosted/deploy/#deployment-types).

### Kubernetes

Kubernetes deployments may be customized in a variety of ways, we consider the following best practice:

-   If Helm cannot be used, [Kustomize can be used to apply configuration changes](/self-hosted/deploy/kubernetes/kustomize), though Kustomize is planned for deprecation. Helm is strongly preferred.
...
-   Be sure your deployment meets our [Docker Compose requirements](/self-hosted/deploy/docker-compose#prerequisites).
-   Review the [configuration section](/self-hosted/deploy/docker-compose/configuration) of our [Docker Compose deployment docs](/self-hosted/deploy/docker-compose/).
...
-   The list of languages currently supported for precise code navigation can be found [here](/code-navigation/auto-indexing)
