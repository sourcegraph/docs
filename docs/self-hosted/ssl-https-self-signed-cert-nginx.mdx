...
While the [OpenSSL](https://wiki.openssl.org/index.php/Command_Line_Utilities) CLI can generate self-signed certificates, its API is challenging unless you're well versed in SSL.
...
Run the new Docker command, then validate by opening your browser at `https://sourcegraph.example.com`.
...
Open your browser again at `https://sourcegraph.example.com` and this time, your certificate should be valid.
...
-   [Configure Sourcegraph's `externalURL`](/admin/config/site-config)
-   [Redirect to external HTTPS URL](/self-hosted/http-https-configuration#redirect-to-external-https-url)
-   [NGINX HTTP Strict Transport Security](/self-hosted/http-https-configuration#redirect-to-external-https-url)
