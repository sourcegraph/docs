...
The setting `max_parallel_workers_per_gather` controls how many _additional_ workers to launch for operations such as parallel sequential scan. We see diminishing returns around four workers per query. Also notice that increasing this value will _multiplicatively_ increase the amount of memory required for each worker to operate safely; doubling this
