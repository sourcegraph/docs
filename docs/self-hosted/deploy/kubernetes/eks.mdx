...
The network configuration principles apply to EKS deployments with the following EKS-specific considerations:

- **EKS-managed VPC**: When using `eksctl create cluster`, a VPC is automatically created with appropriate subnet and route table configuration
- **Node groups**: Place EKS node groups in private subnets for enhanced security
- **Pod networking**: EKS uses the Amazon VPC CNI plugin for pod networking, which assigns VPC IP addresses to pods
- **Security groups**: Configure both cluster security groups and node security groups as defined in the IAM permissions section above

### EKS-Specific Network Configuration

When creating an EKS cluster with custom VPC settings:

```yaml
vpc:
  cidr: 10.0.0.0/16
  nat:
    gateway: HighlyAvailable  # Deploy NAT gateways in multiple AZs
  clusterEndpoints:
    publicAccess: true
    privateAccess: true
  subnets:
...
### EKS-Specific Encryption Considerations

For EKS deployments, additionally configure:

- **Secrets encryption**: Enable envelope encryption for Kubernetes secrets using AWS KMS
- **EBS CSI driver encryption**: When using the EBS CSI driver for persistent volumes, enable encryption in StorageClass definitions

#### Enabling Kubernetes Secrets Encryption
...
# Enable secrets encryption when creating the cluster
eksctl create cluster \
  --name $CLUSTER_NAME \
  --region $REGION \
  --encryption-config kms-key-arn=arn:aws:kms:REGION:ACCOUNT:key/KEY-ID
```

#### EBS Encryption for Persistent Volumes

Create an encrypted StorageClass for persistent volume claims:
