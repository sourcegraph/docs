...
<QuickLinks>
    ...
    <QuickLink title="Introduction" icon='theming' href="#" />
...
Modifying `buildConfig.yaml` unnecessarily can cause errors or unintended behavior. Always check the [configuration docs](/self-hosted/deploy/kubernetes/configure) or comments in [kustomization.yaml](#kustomization-yaml) before changing this file.
...
**Step 3**: You can begin customizing your Sourcegraph deployment by updating the [kustomization.yaml file](#kustomization-yaml) inside your overlay, following our [configuration guides](/self-hosted/deploy/kubernetes/configure) for guidance.
...
See the [deployment docs for Sourcegraph on Kubernetes](/self-hosted/deploy/kubernetes).
