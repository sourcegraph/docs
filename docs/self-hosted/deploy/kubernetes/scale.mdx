...
Increase resources according to the [Scaling Overview per Service](/self-hosted/deploy/scale) if you notice slower search or navigation.

## Cluster resource guidelines

For production environments, we recommend allocate resources based on your [instance size](/self-hosted/deploy/instance-size). See our [resource estimator](/self-hosted/deploy/resource-estimator) for estimates.
...
-   See the docs to understand each service's role:
    -   [Sourcegraph Architecture Overview](/admin/architecture)
    -   [Scaling Overview per Service](/self-hosted/deploy/scale)
