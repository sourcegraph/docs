...
For production deployments, see the [Network Configuration](/self-hosted/deploy/docker-compose/aws#network-configuration) and [Data Encryption](/self-hosted/deploy/docker-compose/aws#data-encryption) sections in the Docker Compose AWS deployment guide.

These sections cover:

- VPC, subnet, security group, Network ACL, and route table configuration
- EBS encryption, S3 encryption, and LUKS setup

For single-container deployments, focus on:

- Basic security group configuration (covered in the deployment steps below)
- Enabling EBS encryption when configuring storage
- Using HTTPS/TLS for securing connections (see [HTTP and HTTPS/SSL configuration](/self-hosted/http-https-configuration))
