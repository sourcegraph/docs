...
<Callout type="info">
	For any questions or issues [get help from an
	engineer](mailto:support@sourcegraph.com).
</Callout>
...
(_Deprecated_) Engagement rate refers to the percentage of users who use Sourcegraph regularly. It is generally used for existing deployments to estimate resources.
...
See our docs on [Persistent data backup in Kubernetes](/self-hosted/deploy/migrate-backup#persistent-data-backup-in-kubernetes) for more detail.

#### How does Sourcegraph scale?

[Click here to learn more about how each Sourcegraph service scales.](/self-hosted/deploy/scale)
