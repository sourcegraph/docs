...
This guide will take you through how to install Sourcegraph with Docker Compose on a server, which could be the local machine, a server on a local network, or cloud-hosted server. You can also follow one of the available _cloud-specific guides_ listed below to prepare and install Sourcegraph on a supported cloud environment:

<QuickLinks>

<QuickLink title="AWS" icon="lightbulb" href="docker-compose/aws" />
<QuickLink title="Azure" icon="theming" href="docker-compose/azure" />
<QuickLink
	title="DigitalOcean"
	icon="presets"
	href="docker-compose/digitalocean"
/>
<QuickLink
	title="Google Cloud"
	icon="installation"
	href="docker-compose/google_cloud"
/>
...
-   Check the [resource estimator](/self-hosted/deploy/resource_estimator) for resource requirements
...
-   <span class="badge badge-beta">optional</span> Configure ingress firewall rules
    to enable secure access to the server
...
Continue with the following steps _after_ you have created a public or private copy of the [deployment repository](https://github.com/sourcegraph/deploy-sourcegraph-docker/):
...
-   This branch will be used to [upgrade Sourcegraph](/self-hosted/deploy/docker-compose/upgrade) and install your Sourcegraph instance.
...
For detailed instructions on how to configure the instance using an override file, please refer to the [configuration docs](/self-hosted/deploy/docker-compose/configuration).
...
-   [Upgrade](/self-hosted/deploy/docker-compose/upgrade)
-   [Management Operations](/self-hosted/deploy/docker-compose/operations)
