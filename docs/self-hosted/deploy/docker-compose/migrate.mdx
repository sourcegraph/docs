...
<Callout type="warning">The single-container (`sourcegraph/server`) deployment mode has been **removed in Sourcegraph 7.0.0**. The `sourcegraph/server` Docker image is no longer published starting with 7.0.0. You **must** migrate to Docker Compose or Kubernetes before upgrading to 7.0.0.</Callout>
