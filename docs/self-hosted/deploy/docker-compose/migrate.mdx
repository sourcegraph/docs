...
<Callout type="warning">The single-container (`sourcegraph/server`) deployment mode has been **removed in [Sourcegraph 7.0.0](https://sourcegraph.com/changelog/releases/7.0)**. The `sourcegraph/server` Docker image is no longer published starting with 7.0.0. You **must** migrate to Docker Compose or Kubernetes before upgrading to 7.0.0.</Callout>
...
-   [Deploy Sourcegraph with Docker Compose on AWS](/self-hosted/deploy/docker-compose/aws)
-   [Deploy Sourcegraph with Docker Compose on Google Cloud](/self-hosted/deploy/docker-compose/google-cloud)
-   [Deploy Sourcegraph with Docker Compose on DigitalOcean](/self-hosted/deploy/docker-compose/digitalocean)
