...
For detailed network configuration including VPCs, subnets, security groups, Network ACLs, and route tables, see the [Network Configuration section in the Docker Compose AWS deployment guide](/self-hosted/deploy/docker-compose/aws#network-configuration).

The network settings configured during deployment (step 6 below) should follow the security group and networking principles outlined in that guide.

---

## Data Encryption

For comprehensive data encryption configuration including EBS encryption, S3 encryption, and LUKS setup, see the [Data Encryption section in the Docker Compose AWS deployment guide](/self-hosted/deploy/docker-compose/aws#data-encryption).

### AMI-Specific Encryption Guidance

When deploying from the Sourcegraph AMI:

- **EBS encryption**: Enable encryption for both the root volume (50GB) and data volume (500GB) in the Configure storage step (step 7 below)
- **Default encryption**: Ensure your AWS account has default EBS encryption enabled before launching the instance
- **KMS keys**: Specify a customer-managed KMS key for enhanced audit capabilities
