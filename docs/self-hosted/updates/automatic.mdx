# Automatic multi-version upgrades (Deprecated)

<Callout type="warning">
**Deprecation Notice:** The autoupgrade feature has been removed in Sourcegraph 6.12.0 and later. Site admins should use the migrator [`upgrade` command](/self-hosted/updates/migrator/migrator-operations#upgrade) for multi-version upgrades instead.

The documentation below is preserved for users on Sourcegraph versions prior to 6.12.0.
</Callout>

---

## Historical Documentation (Sourcegraph 5.1 - 6.11.x)
...
From **Sourcegraph 5.1 through 6.11.x**, multi-version upgrades could be performed **automatically** as if they were a standard upgrade for the same deployment type. Automatic multi-version upgrades took the following general form:
...
1. Determine if your instance is ready to Upgrade:
    ...
    2. Consult upgrade notes for your deployment type across the range of your upgrade. ([Kubernetes](https://sourcegraph.com/changelog/self-hosted/kubernetes), [Docker-compose](https://sourcegraph.com/changelog/self-hosted/docker-compose))
...
For versions prior to v5.0.0, the `SRC_AUTOUPGRADE` environment variable may be set on the `sourcegraph-frontend`, and `migrator` service manifests. To enable automatic upgrades in docker-compose deployments the env var must also be set on `sourcegraph-frontend-internal`. For example,
