...
These failures must be interpreted by the database administrator and resolved using guidance from [How to Troubleshoot a Dirty Database](/self-hosted/how-to/dirty_database). Generally-speaking this will involve looking up the migration source code and manually applying the necessary SQL code.
...
In this case, locate the UP migration 1528395834 and apply all SQL after the final `COMMENT ON EXTENSION` command following the [dirty database procedure](/self-hosted/how-to/dirty_database).
...
These failures must be intepreted by the database administrator and resolved using guidance from [How to Troubleshoot a Dirty Database](/self-hosted/how-to/dirty_database). Generally-speaking this will involve looking up the migration source code and manually applying the necessary SQL code. The `codeintel_schema_migrations` table should be consulted for dirty migrations in this case.
