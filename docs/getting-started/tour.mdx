...
- A [Sourcegraph Cloud](../index) instance, or [deploy and configure Sourcegraph](../admin/deploy/index) and add repositories
- [Install the integration for your code host/review tool](../integration/index)
...
To try this on an open-source pull request, [install the browser extension](../integration/browser_extension) and visit [gorilla/websocket#342](https://github.com/gorilla/websocket/pull/342/files). Use **Find references** to see all cases in which the newly added `AllowClientContextTakeover` is used.
...
- [Deploy and configure Sourcegraph](../admin/deploy/index) and add repositories
- [Install the browser extension and editor plugin](../integration/index) for a faster way to initiate searches (optional)
...
1. Find and click on a search result for the function you're looking for. (If needed, narrow your search using the suggested search filters below the search box or by [adding your filters](../code_search/reference/queries).)
...
1. Find and click on a relevant search result or search suggestion. (If needed, narrow your search using the suggested search filters below the search box or by [adding your filters](../code_search/reference/queries).)
...
1. Scroll through the search results, which show you all commits (and diffs) that match the function name, newest first. (If needed, narrow the diff search by [adding search filters](../code_search/reference/queries).)
