...
The default order can be overridden with an optional `order` parameter. It is an integer and items
...
By default, the `displayPrefix` will be `Continue with` and `displayName` will be inferred from the auth provider type.
