...
The native Kubernetes Executors consists of a master Executor pod which schedules worker pods via the Kubernetes API. The master Executor pod is responsible for managing the lifecycle of jobs, while the worker pods are responsible for processing the actual [batch change](/batch_changes/explanations/server_side) or [precise auto indexing](/code-search/code-navigation/auto_indexing) job specs. For more details, see [how it works](/admin/executors#native-kubernetes).
