...
If [Firecracker isolation will be used](/admin/executors/#how-it-works): _(recommended)_
...
KVM (virtualization) support is required for [our sandboxing model](/admin/executors/#how-it-works) with Firecracker. The following command checks whether virtualization is enabled on the machine (it should print something):
...
If you use [our sandboxing model with Firecracker](/admin/executors/#how-it-works) _(recommended)_, you can also verify that the executor is able to spin up the isolation VMs properly. For that, use the following command:
