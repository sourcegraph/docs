...
| [`searcher`](/admin/deploy/scale#searcher)                     | Provides on-demand un-indexed search for repositories. It fetches archives from gitserver and searches them with regexp. Indexes symbols in repositories using Ctags.                    |
...
<Callout type="warning">
	...
	configuration guide](/admin/config/postgres-conf) for more information.
</Callout>
...
<Callout type="note">
	{' '}
	If Rockskip is enabled, the symbols for repositories indexed by [Rockskip](/code-search/code-navigation/rockskip)
...
<Callout type="note">
	{' '}
	See the docs on [Worker services](/admin/workers#worker-jobs) for a list of worker
	jobs.
</Callout>
