...
1. Follow the instructions on [how to run the migrator job in Docker](/admin/updates/migrator/migrator-operations#docker-compose) to perform the upgrade migratiohn. For specific documentation on the `upgrade` command, see the [command documentation](/admin/updates/migrator/migrator-operations#upgrade). The following specific steps are an easy way to run the upgrade command:
