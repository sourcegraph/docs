...
To use an external Object Storage service (S3-compatible services, or GCS), first review our [general recommendations](/cody/explanations/code_graph_context#storing-embedding-indexes). Then review the following example and adjust to your use case.

<Callout type="note">The example assumes the use of AWS S3. You may configure the environment variables accordingly for your own use case based on our [general recommendations](/cody/explanations/code_graph_context#storing-embedding-indexes).</Callout>
