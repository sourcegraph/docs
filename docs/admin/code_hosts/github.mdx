...
There are two ways to connect a GitHub App: Create a new one via Sourcegraph, which will set up all of the required permissions and automatically retrieve the credentials, or by providing the details of an existing GitHub App (required when using GitHub Enterprise Apps).
