...
To upgrade builtin containers in Helm admins must update their instance to v5.10.0 or v5.11.0 via the [helm multi-version upgrade procedure](/admin/deploy/kubernetes#multi-version-upgrade-procedure) or standard upgrade procedure. This is because `helm upgrade` doesn't support just the DBs in a chart.
