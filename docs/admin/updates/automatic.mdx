...
For versions prior to v5.0.0 the `SRC_AUTOUPGRADE` environment variable may be set on the `sourcegraph-frontend`, and `migrator` service manifests. To enable automatic upgrades. _In docker-compose deployments the env var must also be set on `sourcegraph-frontend-internal`_  Ex:
```yaml
