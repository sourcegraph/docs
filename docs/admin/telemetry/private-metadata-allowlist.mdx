...
| `batchChangeAgent` | `categorization` | `topics` — LLM-extracted topic keywords for the batch spec (e.g. dependency-updates, code-migration); used for product usage reporting. |
...
| `rolePermission` | `assigned` | `permissionIDs` — List of numeric permission IDs assigned to a role; IDs only, needed for RBAC audit analytics. |
| `userRole` | `assigned` | `roleIDs` — List of numeric role IDs assigned to a user; IDs only, needed for RBAC audit analytics. |