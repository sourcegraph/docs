...
Batch Changes is generally configured through the same [site configuration](/admin/config/site-config) and [code host configuration](/admin/code-hosts/) as the rest of Sourcegraph. However, Batch Changes features may require specific configuration, and those are documented here.

## Access control

Batch Changes is [RBAC-enabled](/admin/access-control/) <span class="badge badge-beta">Beta</span>. By default, all users have full read and write access for Batch Changes, but this can be restricted by changing the default role permissions, or by creating new custom roles.
...
Rollout windows are configured through the `batchChanges.rolloutWindows` [site configuration option](/admin/config/site-config). If specified, this option contains an array of rollout window objects that are used to schedule changesets. The format of these objects [is given below](#rollout-window-object).
...
Sourcegraph can track incoming webhooks from code hosts to more easily debug issues with webhook delivery. Learn [how to setup webhooks and configure logging](/admin/webhooks/incoming#webhook-logging).
...
By default, classic and agentic Batch Changes identify the Sourcegraph batch change that created a changeset in two places:

-   A link to the originating batch change appended to the pull request or merge request description.
-   A `Sourcegraph-Batch-Change` trailer in the commit message.

If description updates cause unwanted code host notifications, set `batchChanges.attributionMode` to `trailerOnly` in site configuration:

```json
{
	"batchChanges.attributionMode": "trailerOnly"
}
```

The setting supports these values:

| Value            | Behavior                                                                                    |
| ---------------- | ------------------------------------------------------------------------------------------- |
| `bodyAndTrailer` | Adds attribution to both the changeset description and commit message. This is the default. |
| `trailerOnly`    | Adds attribution only to the commit message, leaving the changeset description unchanged.   |

The commit trailer always contains a link to the classic batch change or agent conversation that created the changeset. If a changeset template explicitly includes `${{ batch_change_link }}`, Sourcegraph honors that template even when `trailerOnly` is configured.
...
<Callout type="note">
	{' '}
	Commit signing on GitHub Enterprise requires [web commit signing](https://docs.github.com/en/enterprise-server@3.14/admin/configuring-settings/configuring-user-applications-for-your-enterprise/configuring-web-commit-signing#enabling-web-commit-signing)
	to be enabled. Otherwise commits from GitHub apps will be unverified.
</Callout>
...
![The Batch Changes settings page on Sourcegraph, scrolled to show commit signing integrations](https://sourcegraphstatic.com/docs/images/administration/config/github-apps/github-apps-batches-list-dark.png)
...
![The GitHub App creation page on Sourcegraph, with the default values filled out](https://sourcegraphstatic.com/docs/images/administration/config/github-apps/github-apps-create-batches-dark.png)

1. When you click **Create GitHub App**, you will be redirected to GitHub to confirm the details of the App to be created.

![The GitHub App creation page on GitHub](https://sourcegraphstatic.com/docs/images/administration/config/github-apps/github-app-create-on-gh-batches-dark.png)
...
![The GitHub App installation page on GitHub](https://sourcegraphstatic.com/docs/images/administration/config/github-apps/github-app-install-gh-batches-dark.png)
...
![The Batch Changes settings page on Sourcegraph, scrolled to show the newly added commit signing integration](https://sourcegraphstatic.com/docs/images/administration/config/github-apps/github-app-post-create-batches-dark.png)
...
![The GitHub App details page on Sourcegraph](https://sourcegraphstatic.com/docs/images/administration/config/github-apps/github-app-details-batches-dark.png)
...
![The GitHub App installation page, with a list of namespaces to select from](https://sourcegraphstatic.com/docs/images/administration/config/github-apps/github-app-multi-install-gh-batches-dark.png)
...
![The GitHub App details page on Sourcegraph, scrolled to show a second new installation](https://sourcegraphstatic.com/docs/images/administration/config/github-apps/github-app-post-multi-install-batches-dark.png)
...
<Callout type="note">
	Templates are supported in Sourcegraph v6.6 and more.
</Callout>
...
<Callout type="note">
	Site admins can manage the Batch Spec library in Sourcegraph v6.4 and more.
</Callout>
...
<Callout type="note">
	Variables are supported in Sourcegraph v6.6 and more.
</Callout>
...
<Callout type="note">
	You cannot use a variable for the batch change name.
</Callout>
