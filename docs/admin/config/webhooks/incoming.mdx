...
Incoming webhooks can be configured on a Sourcegraph instance in order to receive events from code hosts. This allows Sourcegraph to react more quickly to events that occur outside the instance, and decrease the amount of polling for changes required.
...
GitLab webhooks can be installed at the repository or the project level. We strongly recommend using project level webhooks, to decrease the amount of work needed to add webhooks to every repository, and ensure new repositories have the webhooks added as well.
