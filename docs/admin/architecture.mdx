...
Telemetry is exported through the `worker` service to the Sourcegraph-managed "Telemetry Gateway" service.
...
<Callout type="warning">
	These diagrams are oudated, but remain available for historical reference.
</Callout>

### Cody in Sourcegraph Cloud
...
[Cody Enterprise](/cody/clients/enable-cody-enterprise) on [Sourcegraph Cloud](/cloud) is the recommended deployment for Cody Enterprise. It uses the Sourcegraph Cloud infrastructure and is backed by the [Sourcegraph Model Provider (Cody Gateway)](/model-provider).

![Sourcegraph Cloud deployment architecture](https://sourcegraphstatic.com/docs/enterprise-architecture-cloud-v2.png)

### Cody on Amazon Bedrock (self-hosted)

This is an example of a more complex deployment that uses Sourcegraph Enterprise Server (self-hosted) and Amazon Bedrock.

![Sourcegraph Enterprise Server on Amazon Bedrock architecture](https://sourcegraphstatic.com/docs/enterprise-architecture-aws-v0.png)

### Cody Data flow

The following diagram describes the data flow between the different components of Cody Enterprise.

![Cody data flow diagram](https://sourcegraphstatic.com/docs/enterprise-architecture-data-flow-v0.png)
