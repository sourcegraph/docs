...
flowchart TB
    ...
    subgraph Frontend["Frontend Layer"]
        FE["frontend<br/>(GraphQL API)"]
    end

    subgraph Search["Search Services"]
        Searcher["searcher<br/>(unindexed search)"]
        ZoektIndex["zoekt-indexserver"]
        ZoektWeb["zoekt-webserver<br/>(indexed search)"]
        Syntect["syntect-server<br/>(syntax highlighting)"]
    end

    subgraph Backend["Backend Services"]
        Worker["worker<br/>(background jobs)"]
        Gitserver["gitserver<br/>(repository storage)"]
        Symbols["symbols"]
        PCIW["precise-code-intel-worker"]
    end

    subgraph Databases["Data Layer"]
        Postgres[(pgsql<br/>primary DB)]
        CodeIntelDB[(codeintel-db)]
        InsightsDB[(codeinsights-db)]
        Redis[(redis<br/>cache/sessions)]
        Blobstore[(blobstore<br/>object storage)]
    end

    subgraph External["External Services"]
        CodeHosts["Code Hosts<br/>(GitHub, GitLab, etc.)"]
    end
...
flowchart LR
    ...
    subgraph CustomerAWS["Customer AWS Account"]
        subgraph VPC["VPC"]
            SG["Sourcegraph<br/>Enterprise Server"]
            EKS["Amazon EKS"]
        end
...
sequenceDiagram
    ...
    participant Cody as Cody Client
    participant SG as Sourcegraph Instance
    participant Search as Search Services
    participant Gateway as Cody Gateway
    participant LLM as LLM Provider

    User->>Cody: Ask question / Request completion
    Cody->>SG: Send request with context

    SG->>Search: Fetch relevant code context
    Search-->>SG: Return code snippets

    SG->>Gateway: Forward request + context
    Gateway->>LLM: Send prompt to model
    LLM-->>Gateway: Return generated response
    Gateway-->>SG: Forward response

    SG-->>Cody: Return response with context
    Cody-->>User: Display response
```
