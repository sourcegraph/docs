...
Learn more about our [Code host integrations in the docs](/admin/code_hosts).
...
Note that `src-cli` is not contained within the Sourcegraph monorepo and has its release cadence.

Learn more about `src-cli` in the [docs](/cli) and [GitHub repo](https://github.com/sourcegraph/src-cli).
