...
For customers with specific needs, a variety of self-hosted deployment methods are available. Below diagrams are only for reference purposes. Please reach out to your account team for details on the deployment instructions.
...
architecture-beta
    ...
    service s3(aws:s3)[Indexes and Caches] in account

    group vpc(aws:vpc)[VPC] in account
    service alb(aws:alb)[Application Load Balancer] in vpc
    service ebs(aws:ebs)[Data Storage] in vpc

    group public_subnet(aws:public-subnet)[Public Subnet] in vpc
    group eks(aws:eks)[EKS] in public_subnet
    service sg_core(server)[Sourcegraph core] in eks

    group private_subnet(aws:private-subnet)[DB Private Subnet] in vpc
    service rds(aws:rds)[RDS PostgreSQL] in private_subnet
