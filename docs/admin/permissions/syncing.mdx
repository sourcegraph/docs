...
To see which code hosts support permission syncing, please refer to [Supported code hosts table](/admin/permissions#supported-code-hosts).
...
For _user permission sync_, the GraphQL API `syncedAt` indicates the time of last user-centric permission sync for the user
...
For _repository permission sync_, the case is orthogonal to the above. The GraphQL API `syncedAt` indicates the time of last
...
So the answer to the question of how long the _lag-time_ is, in the worst case 125 minutes (as long as the full user-permission syncing cycle takes).
...
> IMPORTANT: Depending on the customer scale, the amount of users, repositories and the distribution of permissions across them, the time it takes to fully sync will vary.
...
means we need to sync 5000 users an hour. If we keep the `syncScheduleInterval` to `60s` (the default), we schedule once
a minute. `5000/60 = 83.3`, so the scheduler needs to schedule 84 users on each iteration to get under the 2 hour mark.
...
need to sync 2500 repositories an hour. If we keep the `syncScheduleInterval` to `60s` (the default), we schedule once
a minute. `2500/60 = 41.7`, so the scheduler needs to schedule 42 repositories on each iteration to get under the 2 hour mark.
...
Learn about how Permission Syncing fits into Sourcegraph in the [architecture overview](/admin/architecture#permission-syncing).
