...
Support for repository permissions across different code hosts is different. The following table captures current state of support (ordered alphabetically):
...
<Callout type="note">
	Sourcegraph might be able to support higher scale than specified, but was
	not rigorously tested to do so. Please contact
	[support@sourcegraph.com](mailto:support@sourcegraph.com) if you want to
	discuss bigger scale than specified.
</Callout>
