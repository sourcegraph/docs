...
<TierCallout>
	Supported on Sourcegraph [Enterprise Starter](/pricing/plans/enterprise-starter) and
...
Replace `$ACCESS_TOKEN` with the access token you are providing to Sourcegraph, and `gitlab.example.com` with your GitLab hostname:

```
curl -H 'Private-Token: $ACCESS_TOKEN' -XGET 'https://gitlab.example.com/api/v4/projects'
```
...
```json
{
	...
	// A list of projects to never mirror from this GitLab instance. Takes precedence over "projects" and "projectQuery" configuration. You can exclude projects by: name ({"name": "group/name"}), ID ({"id": 42}), regular expression matching pattern ({"pattern": "^group\/project-.*"}), by excluding empty repositories ({"emptyRepos": true}), or by excluding projects in personal namespaces ({"personalProjects": true}).
	// Other example values:
	// - [
	//     {
	//       "name": "group/name"
