...
Sourcegraph search defaults to using `fork:no archived:no` as implied search values, excluding [forked](/code-search/queries/language#fork) and [archived](/code-search/queries/language#archived) repositories from search results. If you wish to include forked or archived repositories in your search results you must include the params `fork:yes` or `archived:yes`, respectively.
...
Indexing multiple branches will add additional resource requirements to Sourcegraph (particularly memory). The indexer will deduplicate documents between branches. So the size of your index will grow in relation to the number of unique documents. Refer to our [resource estimator](/self-hosted/deploy/resource-estimator) to estimate whether additional resources are required.
...
When repositories receive updates, Zoekt reindexes them and tombstones their old index data. As a result, compound shards can shrink and be dismantled into individual shards once they reach a critical minimum size. These individual
