...
See "[Code search overview](/code_search/)" for general information about Sourcegraph's code search.

## Indexed search

Sourcegraph indexes the code on the default branch of each repository. This speeds up searches that hit many repositories at once. Not all files in a repository branch are indexed, we skip files that are [larger than 1 MB](/code_search/explanations/search_details) and binary files. To view which files are skipped during indexing, visit the repository settings page and click on indexing.
