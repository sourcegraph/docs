...
To request a [Buildkite build](/dev/background-information/ci/#buildkite-pipelines) for a pull request from a fork, a build must be manually requested after reviewing the contributor's changes. A successful Buildkite build is required for a pull request to be merged.
...
Once changes have been reviewed, a build can be requested directly for a commit with [the `sg` CLI](/dev/background-information/sg/):
...
Then, use [the `sg` CLI](/dev/background-information/sg/) to request a build after reviewing the code:
