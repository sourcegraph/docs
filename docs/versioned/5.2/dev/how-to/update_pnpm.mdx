...
Sourcegraph uses [pnpm](https://pnpm.io/) to handle client-side dependencies and we build our code with [Bazel](/dev/background-information/bazel). We use [aspect.dev](https://www.aspect.dev/)' [aspect_rules_js](https://github.com/aspect-build/rules_js) to integrate it with Bazel.
