...
Instead of [Sourcegraph Cody Gateway](/cody/core-concepts/cody-gateway), admins can also use a third-party embeddings provider like:
...
<Callout type="warning">Azure OpenAI is in experimental stage. It's not recommended to use in a production setting.</Callout>
...
<QuickLinks>
	<QuickLink href="/cody/embeddings/embedding-index" icon="installation" imgAlt="generate-embedding-index" title="Generating an Embeddings Index" description="This guide will explain how to generate an embeddings index that enables codebase-aware answers for Cody." />
	<QuickLink href="/cody/embeddings/configure-embeddings" icon="lightbulb" imgAlt="configure-embeddings" title="Configure Embeddings" description="Learn how you can configure embeddings with Cody for better code context." />
	<QuickLink href="/cody/embeddings/manage-embeddings" icon="presets" imgAlt="manage-embeddings" title="Manage Embeddings" description="Learn how you can manage embeddings via filtering and storing embedding indexes." />
	<QuickLink href="/cody/embeddings/usage-and-limits" icon="theming" imgAlt="usage-limits" title="Usage & Limits" description="Learn about all the usage polices and limits that apply while using embeddings." />
</QuickLinks>
