...
<p className="subtitle">Learn how you can manage embeddings via filtering and storing embedding indexes.</p>
...
<Callout type="note">The `excludedFilePathPatterns` setting is only available in Sourcegraph version `5.0.1` and later.</Callout>
...
<Callout type="warning">Make sure that you never commit your AWS access keys in Git. Instead, consider using a secret handling service offered by your cloud provider.</Callout>
...
<Callout type="note">You don't need to set the `EMBEDDINGS_UPLOAD_AWS_ACCESS_KEY_ID` environment variable when using `EMBEDDINGS_UPLOAD_AWS_USE_EC2_ROLE_CREDENTIALS=true` because role credentials will be automatically resolved.</Callout>
