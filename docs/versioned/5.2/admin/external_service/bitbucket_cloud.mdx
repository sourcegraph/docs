...
{/* <div markdown-func=jsonschemadoc jsonschemadoc:path="admin/external_service/bitbucket_cloud.schema.json">[View page on docs](https://sourcegraph.com/docs/admin/external_service/bitbucket_cloud) to see rendered content.</div> */}
...
```json
{
	...
	// If non-null, enforces Bitbucket Cloud repository permissions. This requires that there is an item in the [site configuration json](/admin/config/site_config#auth-providers) `auth.providers` field, of type "bitbucketcloud" with the same `url` field as specified in this `BitbucketCloudConnection`.
	"authorization": null,
