...
Using the [`steps.outputs.<name>.format`](/batch_changes/references/batch_spec_yaml_reference#steps-outputs-name-format) field, it's possible to parse the value of an output as JSON or YAML and access it as a data structure instead of just text:
