...
In both the short-term and the long-term, it is far easier for users to [connect Sourcegraph to a code host](/admin/external_service) than to try to load code from a local disk, and we strongly encourage users, where possible, to connect to their code host of choice instead of trying to follow the instructions below.
