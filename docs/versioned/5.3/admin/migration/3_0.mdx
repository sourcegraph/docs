...
Top level configuration options for code hosts (e.g. "github", "gitlab", "phabricator", "awscodecommit", "bitbucket", "gitolite") have been removed from [site configuration](/admin/site_config/all).
...
The top-level `repos.list` site configuration was removed in favour of each code-host's equivalent options, now configured via [external services](#Repositories-are-managed-by-configuring-external-services). Equivalent options in code hosts configuration:
  - GitHub via [`github.repos`](/admin/site_config/all#repos-array)
  - GitLab via [`gitlab.projectQuery`](/admin/site_config/all#projectquery-array)
  - Phabricator via [`phabricator.repos`](/admin/site_config/all#phabricator-array)
  - [Other external services](/admin/repo/add_from_other_external_services)
