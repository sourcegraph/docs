...
Instead of computing Batch Changes locally using `src-cli`, you can offload this task to one or many remote server called an [executor](/admin/executors/deploy_executors). Executors are also required to enable code navigation [auto-indexing](/code_navigation/explanations/auto_indexing).
