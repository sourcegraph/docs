...
You can add guidance on the **Agents** page of your Batch Changes settings, at `sourcegraph.example.com/users/<username>/settings/batch-changes/agents`. Site administrators add instance-wide guidance at `sourcegraph.example.com/admin/batch-changes/agents`, and organizations can carry shared guidance in their settings. Entries from all three levels combine.
...
Before drafting a plan, the agent researches with [Deep Search](/deep-search). Deep Search usage consumes subscription credits separately from the agentic batch change’s own metering.
...
Agentic Batch Changes link generated changesets back to their agent conversation in both the changeset description and commit message by default. Site administrators can [configure pull request attribution](/admin/config/batch-changes#pull-request-attribution) to keep this link out of descriptions that may trigger unwanted code host notifications.
