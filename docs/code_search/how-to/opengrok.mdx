...
- [All Sourcegraph search documentation](/code_search/)
  - [Sourcegraph query syntax](/code_search/reference/queries)
...
Sourcegraph, [provides full regular expression search](/code_search/reference/queries#regexp-search), with support for the [RE2 syntax](https://golang.org/s/re2syntax). The same search above would take the form `foo.*` (or in this case, just `foo`, since Sourcegraph automatically supports partial matches). Much more powerful regexp expressions are available.

(Note that Sourcegraph also provides a [literal search mode](/code_search/reference/queries#Literal-search-default) by default, in which there's no need to escape special characters. This simplifies searches such as `foo(`, which would result in an error in regexp mode.)
...
Sourcegraph also allows site admins to create pre-defined repository groupings, using [version contexts](/code_search/explanations/features#version-contexts-experimental).
...
Sourcegraph indexes all characters, and can search for strings of any length. Using the default [literal search mode](/code_search/reference/queries#Literal-search-default), any search (including those with special characters like `foo.bar`, `try {`, `i++`, `i-=1`, `foo->bar`, and more), will all be searchable without special handling. Using [regexp mode](/code_search/reference/queries#regexp-search) would require escaping special charactes.
...
Sourcegraph also provides [`AND`, `OR`, and `NOT` operators](/code_search/reference/queries#boolean-operators).
...
Sourcegraph also provides keywords to [scope commit message and diff searches](/code_search/reference/queries#keywords-diff-and-commit-searches-only) to specific authors or timeframes in which a change was made.

To see an exhaustive list of Sourcegraph's search keywords, see the [search query syntax](/code_search/reference/queries#keywords-all-searches) page.
