...
-   The private DNS name of the private resource, e.g. `github.internal.example.com`. Note: this is the DNS name customer users interact on a daily basis.
-   The public DNS name of the network load balancer exposing the private resource, e.g. `github-public-nlb.example.com`.
...
```sh
curl --connect-to github.internal.example.com:443:github-public-nlb.example.com:443 https://github.internal.example.com
```
...
Once the connection is established, the customer can create the [code host connection](/admin/code-hosts/) on their Sourcegraph Cloud instance using private dns name.
