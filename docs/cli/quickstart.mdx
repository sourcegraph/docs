...
```powershell
$SrcInstallDir = "$env:ProgramFiles\Sourcegraph"
New-Item -ItemType Directory -Path $SrcInstallDir -Force | Out-Null

Invoke-WebRequest https://sourcegraph.com/.api/src-cli/src_windows_amd64.exe -OutFile "$SrcInstallDir\src.exe"

$MachineEnvironment = Get-Item 'HKLM:\SYSTEM\CurrentControlSet\Control\Session Manager\Environment'
$MachinePath = $MachineEnvironment.GetValue('Path', '', 'DoNotExpandEnvironmentNames')
$InstallPathPattern = [regex]::Escape('%ProgramFiles%\Sourcegraph')
$MachinePathEntries = @($MachinePath -split ';' | Where-Object { $_ -notMatch "^$InstallPathPattern\\?$" })
$MachinePathEntries += '%ProgramFiles%\Sourcegraph'
Set-ItemProperty -LiteralPath $MachineEnvironment.PSPath -Name Path -Value ($MachinePathEntries -join ';') -Type ExpandString -ErrorAction Stop

if (($env:Path -split ';') -notContains $SrcInstallDir) {
    $env:Path += ";$SrcInstallDir"
}
```
