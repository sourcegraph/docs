...
`src` needs to be authenticated against your Sourcegraph instance. The quickest way to do this is to add `SRC_ENDPOINT=https://sourcegraph.example.com` to your shell environment, then run `src login` to launch OAuth authentication against your Sourcegraph instance.
