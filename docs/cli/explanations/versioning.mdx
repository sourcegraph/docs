...
or, if you know the specific version to target, for example 5.6.2535:

```bash
brew install sourcegraph/src-cli/src-cli@5.6.2535
```

or with npm/npx:

```bash
npx @sourcegraph/src@5.6.2535 version
```
...
or, with npm/npx, if you know the specific version to target, for example 5.6.2535:
...
`sourcegraph/src-cli` is published to Docker Hub. You can use the `latest` tag or a specific version such as `5.6.2535`. To see all versions view [sourcegraph/src-cli tags](https://hub.docker.com/r/sourcegraph/src-cli/tags).
