...
**Deprecated:** As of Sourcegraph 7.1.0, we no longer generate SBOMs as part of the release process. The `src sbom fetch` command will only work for Sourcegraph releases between 5.9.0 and 7.0.x. For questions about software supply chain security, please contact your Sourcegraph account team.
</Callout>

Sourcegraph generated and cryptographically signed a Software Bill of Materials (SBOM) for each container image in releases 5.9.0 through 7.0.x. These SBOMs are stored in our container registry alongside their corresponding images.
...
1. Determine the Sourcegraph version to verify. Use either a [recent release](https://sourcegraph.com/changelog/releases) or your instance's current version.
    ...
    > **Note:** SBOMs are only available for Sourcegraph releases 5.9.0 through 7.0.x.
