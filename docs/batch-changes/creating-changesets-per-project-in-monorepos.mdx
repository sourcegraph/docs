...
Large repositories often contain multiple projects, hence named as **Monorepos**. It can make sense to run the batch spec [`steps`](/batch_changes/references/batch_spec_yaml_reference#steps) separately in each project and create one changeset per project.

This can be done by using [`workspaces`](/batch_changes/references/batch_spec_yaml_reference#workspaces) in the batch specs via two steps:
...
The [`workspaces`](/batch_changes/references/batch_spec_yaml_reference#workspaces) property in batch specs allows you to do that:
...
The `steps.path` [templating variable](/batch_changes/references/batch_spec_templating) contains the path in which the `steps` were executed relative to the root of the repository.
...
The `workspace` property leverages Sourcegraph search to find the location of the defined workspaces in the repositories yielded by the [`on`](/batch_changes/references/batch_spec_yaml_reference#on) property of the batch spec.
...
If the repository containing the workspaces is really large and it's not feasible to download to make it available for the `steps` execution, the [`workspaces.onlyFetchWorkspaces`](/batch_changes/references/batch_spec_yaml_reference#workspaces-onlyfetchworkspace) field can be set to `true` to only download the workspaces, without the rest of the repository.

<Callout type="info">To learn more about `workspaces`, read in the [batch spec YAML reference](/batch_changes/references/batch_spec_yaml_reference#workspaces).</Callout>
