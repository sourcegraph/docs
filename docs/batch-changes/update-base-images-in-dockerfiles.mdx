...
![update_base_images_in_dockerfiles_teaser](https://sourcegraphstatic.com/docs/images/batch_changes/update_base_images_in_dockerfiles_teaser.png)
...
- Feel free to then publish the changesets (i.e., create pull requests and merge requests) by [modifying the `published` attribute in the batch spec](/batch_changes/references/batch_spec_yaml_reference#changesettemplate-published) and re-running the `src batch preview` command.
