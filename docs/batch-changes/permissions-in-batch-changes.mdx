...
Batch changes can be created under the user's or an organization's namespace. If the organization is configured to allow [member administration of organization-namespaced batch changes](admin/config/batch_changes#enable-organization-members-to-administer) all organization users have admin access to the batch change if placed under an organization namespace.
