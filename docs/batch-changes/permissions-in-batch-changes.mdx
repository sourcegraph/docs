...
These levels apply on top of [role-based access control](/admin/access-control/batch-changes): a user needs the `batch_changes:read` permission to view any batch change and the `batch_changes:write` permission to create or execute one. See [Restricting Batch Changes to specific users](#restricting-batch-changes-to-specific-users).
