...
- For each `repositoriesMatchingQuery`, it will:
  ...
  - (Optional): If the results are file matches, then their path in the repository is also saved so that they can be used in the `steps` with [templating](/batch_changes/references/batch_spec_templating)
