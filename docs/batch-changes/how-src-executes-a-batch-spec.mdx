...
This local workflow is distinct from [server-side execution](/batch-changes/server-side). For most server-side jobs, Sourcegraph services resolve and orchestrate the work, while the internal `batch-exec` binary handles only the execution of batch-spec steps within an executor workspace.
