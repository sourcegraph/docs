...
You can do so by changing the [`on.repositoriesMatchingQuery`](/batch_changes/references/batch_spec_yaml_reference#on-repositoriesmatchingquery) to yield fewer results or by using a concrete list of repositories with [`on.repository`](/batch_changes/references/batch_spec_yaml_reference#on-repository).
