...
-   Configure any desired optional features, such as:
    ...
    -   [Pull request attribution](/admin/config/batch-changes#pull-request-attribution), which controls whether Sourcegraph adds a link to changeset descriptions or attributes them only through commit trailers
...
-   [Restrict Batch Changes to specific users](/batch-changes/permissions-in-batch-changes#restricting-batch-changes-to-specific-users) using [role-based access control](/admin/access-control/batch-changes)
-   [Restrict merging changesets to site admins](/batch-changes/permissions-in-batch-changes#restricting-merges-to-site-admins)
