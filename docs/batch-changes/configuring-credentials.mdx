...
Two types of access tokens can be configured to use with Batch Changes:
...
2. [**Global service account token**](#global-service-account-tokens) (Configurable by admins only) - A token that can be used by any Batch Changes user who does not have a personal access token configured. These are also required for [importing changesets](/batch-changes/tracking-existing-changesets) and syncing changeset state from the code host when webhooks are not configured. Although currently importing changesets is not supported without a global access token, if you’re concerned about individual user permissions, using a global access token is not recommended.

Different tokens are used for different types of operations, as shown in the table below.
...
When creating a changeset on a code host, the author of the changeset will reflect the token used (e.g., on GitHub, the user will be the pull request author). This is why a personal access token is preferred for most operations.
...
### Adding a personal GitHub app

Adding a personal GitHub app is done through the Batch Changes section of your user settings:
...
### Removing a personal GitHub app

To remove a GitHub app from your Sourcegraph instance, navigate back to the same section of your user settings, then click **Remove**. Once the GitHub app is removed, the code host's indicator should revert to the empty red circle.
...
### Adding a global GitHub app

Adding a global service account GitHub app is done through the Batch Changes section of the site admin area:
...
### Removing a global GitHub app

To remove a GitHub app, navigate to the same section of the site admin area, then click **Remove**. Once the app is removed, the code host's indicator should revert to the empty red circle.
