...
- The forking mechanism (`fork:true`) is only supported if the GitHub app has access to all repositories of the installation.
- The forking mechanism (`fork:true`) is only supported if the origin repository and the fork are in the same organization that the GitHub app is installed to.
