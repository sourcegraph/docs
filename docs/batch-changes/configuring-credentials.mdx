...
- Sourcegraph instance with repositories in it. Read more in the [Quickstart](/batch_changes/how-tos/#quick-install) guide on how to set up a Sourcegraph instance
...
- Click **Add credentials** and follow the steps to [create a new token](#creating-a-code-host-token) for the code host
...
However, a global service account token is currently required for [importing existing changesets](/batch_changes/how-tos/tracking_existing_changesets) on your code hosts into batch changes.
...
![user-token-input](https://sourcegraphstatic.com/docs/images/batch_changes/user-token-input.png)
