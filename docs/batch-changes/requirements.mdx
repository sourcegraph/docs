...
<p className="subtitle">This document describes the requirements for running Batch Changes.</p>
...
For Sourcegraph to interface with these, admins and users must first [configure credentials](/batch_changes/how-tos/configuring_credentials) for each relevant code host.
