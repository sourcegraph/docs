...
<Callout type="tip">It's recommended to read [Getting started with running batch changes server-side](/batch_changes/explanations/server_side_getting_started) for better understanding.</Callout>
...
File [`mounts`](/batch_changes/references/batch_spec_yaml_reference#steps-mount) are a powerful way to run custom files without directly embedding the files in your batch spec.
...
To use the Python script in your batch change, mount the script in a `step` using the [`mounts`](/batch_changes/references/batch_spec_yaml_reference#steps-mount) field. The following is an example of mounting
