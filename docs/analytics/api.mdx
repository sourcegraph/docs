...
### Getting Started
> **_NOTE:_** As of July 2025, the authentication cookie has changed from `cas` to `accounts_session_v2` (SAMS session cookie). If you have existing scripts using the old cookie name, please update them accordingly.

Access tokens are created using the Sourcegraph Accounts (SAMS) session cookie (`accounts_session_v2`) for authentication to the token creation endpoint. Access tokens are longer lived than the SAMS session cookie making them more suitable for programmatic access to the Sourcegraph Analytics API.
