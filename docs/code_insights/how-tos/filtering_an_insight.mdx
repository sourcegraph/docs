...
In the `context:` field of the filter panel, you can use a [query-based search context](/code_search/how-to/search_contexts#beta-query-based-search-contexts) to filter your insights to only results matching repositories that match the query-based context's `repo:` keyword.

First, if you haven't already created a search context, create a [query-based search context](/code_search/how-to/search_contexts#beta-query-based-search-contexts). You can define any group of repos using the syntax `repo:(^github\.com/sourcegraph/sourcegraph$|^github\.com/sourcegraph/about$...)`.
