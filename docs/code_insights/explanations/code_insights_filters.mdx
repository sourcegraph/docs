...
You can include or exclude repositories from your insight using regular expressions, the same way you can with the `repo:` filter in [Sourcegraph searches](/code_search/reference/queries#keywords-all-searches).
...
You can use a [query-based search context](/code_search/how-to/search_contexts#beta-query-based-search-contexts) to filter your insights to only results matching repositories that match the `repo:` or `-repo:` filter of the query-based context.
