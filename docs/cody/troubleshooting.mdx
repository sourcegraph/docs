...
If you are logged into Sourcegraph.com, only public open source repositories listed [here](embedded-repos) are indexed. To have your open source repository added to the public index, join the [Sourcegraph Discord](https://discord.gg/8wJF5EdAyA) and ask in the `#embeddings-indexing` channel.

If you’re connected to a Sourcegraph Enterprise instance, ask your site admin to [Configure Code Graph Context](core-concepts/code-graph) for your Sourcegraph instance. Then, [enable Cody](cody-clients/enable-cody-enterprise) for your account.
...
For more information, see [Generate Index to Enable Codebase-Aware Answers](embeddings/embedding-index).
