...
<p className="subtitle">Learn how to install the <code>cody</code> command-line tool.</p>
...
<LinkCards>
   <LinkCard href="https://www.npmjs.com/package/@sourcegraph/cody" imgSrc="https://storage.googleapis.com/sourcegraph-assets/docs/images/cody/command-line-blue.svg" imgAlt="Cody CLI" title="Cody CLI" description="Install Cody CLI from npm" />
</LinkCards>
...
It may be tempting to export these variables in your shell startup script, but this is not recommended because it will expose your access token to all commands you run from the terminal.
Instead, consider sourcing these environment variables on-demand when you need to authenticate with the Cody CLI.
...
<p className="subtitle">Learn how to use the <code>cody chat</code> subcommand.</p>
...
## cody chat --context-repo

Use `--context-repo` to provide context from remote repositories.
This feature is only available in Cody Enterprise.
