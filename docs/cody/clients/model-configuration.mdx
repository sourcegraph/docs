...
<Callout type="note">For Sourcegraph Cloud customers, configuring the available LLM models requires contacting Sourcegraph's tech support.</Callout>

Starting with Sourcegraph 5.5.4+ Sourcegraph Enterprise supports the ability to choose between different LLM models, allowing developers to use the best model for their current needs.
...
You can leave these settings blank and have reasonable defaults be chosen for you, or you can express the models explicitly.
...
<Callout type="warning">Defining your own LLM providers and models is an advanced use-case and requires care to get the correct results. It also may bypass protections to ensure compatibiloity between your Sourcegraph instance and LLMs. If you need help contact your Sourcegraph account executive.</Callout>
...
**modelRef**. Each model is given a unique identify, referred to as a model reference or "mref". This is a string of the form `${providerId}::${apiVersionId}::${modelId}`.
...
# Older style "Completions" Configuration
