...
Regarding testing with other coder models for the autocomplete use cases, StarCoder offers significant improvements in quality and latency for users on Sourcegraph.com. You can read more about the improvements in [October 2023 release notes](https://sourcegraph.com/blog/feature-release-october-2023) and the [GA release notes](https://sourcegraph.com/blog/cody-is-generally-available).

To get the best Cody autocomplete experience, Enterprise users can set up an enterprise-grade deployment with dedicated hardware with [Fireworks](https://fireworks.ai/). Since Sourcegraph 5.3, every Cody setup that uses [Cody Gateway](/cody/core-concepts/cody-gateway) can opt-in to using [StarCoder](https://huggingface.co/blog/starcoder) as an alternative to Claude Instant as the AI model for autocomplete.
...
It's recommended that every instance admin makes this change and plans to make this the default in a future release.
