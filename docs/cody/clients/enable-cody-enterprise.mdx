...
- **Multi-repo context**: Cody can fetch information from multiple repositories, enabling users to ask broader questions across a codebase. You can select of upto **10 repos** from the [Enhanced Context Selection](/cody/clients/install-vscode#enhanced-context-selector) window in VS Code
- **Cody ignore**: Teams can flag repositories and files to exclude them from Cody's context, safeguarding sensitive information
- **Guardrails for public code**: Cody scans suggestions to detect code similar to public indexes, preventing its return to users or highlighting it in chat responses
- **Enterprise analytics**: Teams have direct access to analytics for Cody usage data, gaining insights into AI usage and value delivered
- **Improved autocomplete with StarCoder**: StarCoder, a dedicated language model for code, enhances autocomplete speed and accuracy on Cody Enterprise
- **Admin controls**: Sourcegraph account admins now have selective control over user access to Cody
