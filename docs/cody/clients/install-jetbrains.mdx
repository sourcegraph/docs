...
JetBrains users on the Free or Pro plan can leverage [symf](/cody/core-concepts/context#context-selection) (local keyword search) as the primary context source for Cody chat. Local or remote embeddings will no longer be produced or used as a context source.
...
JetBrains users on the Free or Pro plan get single-repo support and can use one repo for context fetching. Enterprise users get multi-repo support and can explicitly specify **up to 10 additional repos** they would like Cody to use for context.
