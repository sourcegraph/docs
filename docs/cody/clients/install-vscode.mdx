...
VS Code users on the Free or Pro plan can leverage [symf](/cody/core-concepts/context#context-selection) (local keyword search) as the primary context source for Cody chat with access to local embeddings. Remote embeddings will no longer be produced or used as a context source.
