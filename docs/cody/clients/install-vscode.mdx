...
<Callout type="info" title="Note">Cody Natural Language Search is currently available in Experimental for all users on VS Code extension.</Callout>
...
To use Cody with code graph on private code, it's recommended to [enable Cody for Enterprise](/cody/clients/enable-cody-enterprise).
