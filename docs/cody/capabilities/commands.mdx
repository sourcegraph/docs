...
<p className="subtitle">
  Learn how Commands help you kick-start with reusable prompts.
</p>
...
The support for one command may vary by IDE extension. Read the [feature parity reference to learn what commands are available in your IDE](/cody/clients/feature-reference#commands).

<Callout type="note">Cody's responses to commands might be better than responses to chats since they've been pre-packaged and prompt-engineered.</Callout>
...
<Callout type="note">
Custom Commands are in the experimental stage.
</Callout>
...
```json
{
  "latest-cody-release": {
    ...
    "context": {
      "selection": false,
      "command": "curl https://raw.githubusercontent.com/sourcegraph/cody/main/vscode/CHANGELOG | head -n 50"
    }
  }
}
```
...
- Type: `string`
- Default: `""`
- Example: `"CHANGELOG", "test/unit/example.test.ts"`
