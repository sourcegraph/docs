...
<Callout type="info">The `@` mention menu is available in the prompt editor starting with Sourcegraph 6.0.</Callout>
...
When selecting a Web URL, please type out the domain including the https:// prefix, for example https://sourcegraph.com.
...
To add dynamic context, please click on one of the buttons below the prompt editor. We will soon move the buttons into the `@` mention menu as well.

This powerful combination allows prompt authors to create templates that can intelligently access both explicitly defined context as well as contextually relevant information at runtime.
