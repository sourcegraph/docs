...
<Callout type="info">These options are only available for VS Code extension users.</Callout>
...
<Callout type="info">Commands are only supported in the VS Code and JetBrains extension.</Callout>
