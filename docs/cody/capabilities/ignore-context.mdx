...
1.  Having Cody Enterprise license and running a Sourcegraph version `>=5.4.0`
2.  Using Cody client with Cody context filters support: Cody Web, Cody VS Code Extension, Cody JetBrains Extension. Any other client won't work if Cody context filters are defined.
3.  Cody client version satisfies the following version constrains: `>=1.20.0` for the VS Code and `>=6.0.0` for the JetBrains. Any earlier client versions won't work if Cody context filters are defined.
4.  Setting "cody-context-filters-enabled" feature flag to `true`.
