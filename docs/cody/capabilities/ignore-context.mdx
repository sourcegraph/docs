...
The `include` and `exclude` rules define the repositories Cody can use for context filtering to third-party LLMs. The rules can be defined in the following combination:
...
## Context Filters Compatibility Matrix

<Callout type="info"> Cody Context Filters work only with Sourcegraph versions >=5.4.0 for Enterprise users.</Callout>

Depending on the client type, here's a detailed breakdown of all the prerequisites that are supported for the context filters to work.

|        **Client Versions**        | Sourcegraph `v<5.4.0` | Sourcegraph `v=5.4.0`                           | Sourcegraph `v=5.4.X`                           | Sourcegraph `v>=5.4.0` |
| --------------------------------- | --------------------- | ------------------------------------------------------------------------ | ------------------------------------------------------------------------- | ---------------------- |
| **JetBrains `<= 5.5.9`**          | N/A          | Respects policy, but the server returns an unhelpful error | Respects policy, the server returns compatibility check and prompt to upgrade | Not affected           |
| **`5.5.10 <= JetBrains < 6.0.0`** | N/A          | Respects policy, but the server only returns a compatibility check           | Respects policy, the server returns compatibility check and prompts to upgrade | Not affected           |
| **JetBrains `>= 6.0.0`**          | N/A          | Respects policy                                                          | Respects policy                                                           | Not affected           |
| **VS Code `< 1.16.X`**            | N/A          | Respects policy, but the server returns an unhelpful error | Respects policy, the server returns compatibility check and prompt to upgrade | Not affected           |
| **`1.18.0 < VS Code < 1.20.0`**   | N/A          | Respects policy, but the server only returns a compatibility check           | Respects policy, the server returns compatibility check and prompt to upgrade | Not affected           |
| **VS Code `>= 1.20.0`**           | N/A          | Respect policy                                                           | Respects policy                                                           | Not affected           |
