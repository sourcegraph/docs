...
Out of the box, Sourcegraph is already aware of many models from the following providers:
...
### Overriding configuration of a model provider
...
-   Ensure that at least one model is available for each Cody feature ("chat" and "autocomplete"), regardless of
...
-   Add two Fireworks models:
    -   `"fireworks::v1::mixtral-8x22b-instruct"` with "chat" capabiity - used for "chat"
        and "fastChat"
...
-   Add the `"google::v1::gemini-1.5-pro"` model, which is used for all Cody features.
...
For detailed information about configuring self-hosted models, please refer to the [self-hosted models](/cody/model-configuration#self-hosted-models) documentation.
