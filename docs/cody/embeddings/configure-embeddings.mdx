...
If you choose not to define a pattern, the policy will be applied to up to [embeddings.policyRepositoryMatchLimit](./usage-and-limits#configure-global-policy-match-limit) repositories.
...
- If another job for the same repository has been queued for processing within the [embeddings.MinimumInterval](./../embeddings#minimum-time-interval-between-automatically-scheduled-embeddings) time window
...
Whether created manually or through a policy, embeddings will be generated incrementally if [incremental updates](./../embeddings#incremental-embeddings) are enabled.
