...
## Introduction
This guide will walk you through setting up HTTP, HTTPS, and SOCKS proxies in Visual Studio Code (VS Code) and JetBrains IDEs. It also includes instructions for handling self-signed certificates on macOS and Windows.
...
### Restart VS Code
After modifying the `settings.json` file, completely close and restart VS Code to apply the new proxy settings.
...
**Note:**
1. Only `Manual Proxy Configuration` is supported at the moment. The `auto-detect proxy settings` option is not fully supported.
...
<Callout type="info"> Please confirm with your IT Administrator to ensure your trusting the right certificate.</Callout>

**Note:** When adding self-signed certificates, try making a `curl` request routed through the HTTPS proxy to check if the self-signed certificate is actually used by the computer. Your request will look something like:
...
## Additional Support
For further assistance with proxy configurations, especially for special enterprise setups, please reach out to the Code support team. They can provide guidance tailored to your specific network environment.
