...
- You have enabled an instance for [Cody from your Sourcegraph.com](/cody/cody-clients/cody-with-sourcegraph) account
...
After connecting Cody extension to Sourcegraph.com, you can optionally use [Code Graph Context](/cody/core-concepts/code-graph) to improve Cody's context of existing code. Note that Code Graph Context is only available for public repositories on Sourcegraph.com, which have embeddings.

You can view the [list of repositories with embeddings here](/cody/embedded-repos). To add any of these to your dev environment, contact a Sourcegraph team member via [Discord](https://discord.gg/8wJF5EdAyA) to get help with the access and setup.

To use Cody with code graph on private code, it's recommended to [enable Cody for Enterprise](/cody/cody-clients/enable-cody-enterprise).
