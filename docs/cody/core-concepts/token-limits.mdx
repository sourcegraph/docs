...
For users of `Claude 3 Sonnet` or `Opus` models, Cody supports up to 30,000 tokens (~4,000 LOC) of user defined context through @-mention syntax and 15,000 tokens of continued chat history (user queries + system responses).  All other models are currently capped at 7,000 tokens of shared context between user defined context and chat history.  
