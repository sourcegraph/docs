...
**Currently Cody caps inputs to approximately 30,000 tokens (using the @-mention syntax) and outputs to approximately 4,000 tokens.**  This balances our needs for context space, retrieval accuracy and latency ([more here](/cody/core-concepts/token-limits#tradeoffs-size-accuracy-latency-and-cost)).  We are actively working to increase these limits, and will update this page when we do.
