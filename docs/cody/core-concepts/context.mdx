...
- **Symf**: Sparse vector search running on a user's machine. Creates a locally computed and stored search index of an active repo. Queries are converted to an expanded set of keywords, which are used to retrieve relevant documents from the index
...
|    **Tier**    | **Client** | **Symf** | **Search** |     **Embeddings**      |
| -------------- | ---------- | -------- | ---------- | ----------------------- |
...
Cody's context window represents the amount of contextual information provided to assist in generating code or responses. Currently, the context window size for both chat and commands is set to **8000 tokens per interaction**.
