...
| `USE_ROCKSKIP`     | `false`                              | Enables [Rockskip](/code_navigation/explanations/rockskip) for fast symbol searches and search-based code navigation on repositories specified in `ROCKSKIP_REPOS`, or respositories over `ROCKSKIP_MIN_REPO_SIZE_MB` in size                               |
| `ROCKSKIP_REPOS`     | N/A                              | In combination with `USE_ROCKSKIP=true` this specifies a comma-separated list of repositories to index using [Rockskip](/code_navigation/explanations/rockskip) (e.g. `github.com/torvalds/linux,github.com/pallets/flask`)                              |
...
| `MAX_CONCURRENTLY_INDEXING`     | 4                              | Maximum number of repositories being indexed at a time by [Rockskip](/code_navigation/explanations/rockskip) (also limits ctags processes)                              |
...
<Callout type="tip">To enable precise code navigation for your repository, see our [docs here](/code_navigation/how-to/).</Callout>
