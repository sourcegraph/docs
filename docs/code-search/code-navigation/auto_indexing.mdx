...
With Sourcegraph deployments supporting [executors](/admin/executors/), your repository contents can be automatically analyzed to produce a code graph index file. Once [auto-indexing is enabled](/code_navigation/how-to/enable_auto_indexing) and [auto-indexing policies are configured](/code_navigation/how-to/configure_auto_indexing), repositories will be periodically cloned into an executor sandbox, analyzed, and the resulting index file will be uploaded back to the Sourcegraph instance.

## Enable Code Navigation auto-indexing

<Callout type="note">This feature is in beta for self-hosted customers.</Callout>

The following docs explains how to turn on [auto-indexing](/code_navigation/explanations/auto_indexing) on your Sourcegraph instance to enable [precise code navigation](/code_navigation/explanations/precise_code_navigation).
...
Once auto-indexing has been enabled, [create auto-indexing policies](/code_navigation/how-to/configure_auto_indexing) to control the set of repositories and commits that are eligible for indexing. Note that policies only select repositories and branches/tags/commits that can be indexed, not how they are indexed.

### Configure auto-indexing jobs

For repository and commit pairs that are marked as eligible for indexing, the index job is either [inferred from the project](/code_navigation/explanations/auto_indexing_inference), or [explicitly configured](/code_navigation/how-to/configure_auto_indexing#explicit-index-job-configuration) on a per-repository basis.
...
The type of information you will need to provide depends on the language and ecosystem, but the mechanism is the same - [adding a secret to Executors "Code Graph" namespace](/admin/executors/executor_secrets).

See the [language-specific instructions](/code_navigation/how-to/ configure_auto_indexing#private-repositories-and-packages-configuration)

## Configure Code Navigation auto-indexing

<Callout type="info">This feature is in beta for self-hosted customers.</Callout>

Precise code navigation [auto-indexing](/code_navigation/explanations/auto_indexing) jobs are scheduled based on two fronts of configuration.
...
The second front determines the set of index jobs that can run over candidate commits. By default, index jobs are [inferred](/code_navigation/explanations/auto_indexing_inference) from the repository structure's on disk. Index job inference uses heuristics such as the presence or contents of particular files to determine the paths and commands required to index a repository.
...
Let's learn how to configure policies to control the scheduling of precise code navigation indexing jobs. Indexing jobs [produce a code graph data index](/code_navigation/explanations/precise_code_navigation) and uploads it to your Sourcegraph instance for use with code navigation.
...
For projects that have non-standard or complex dependency resolution or pre-compilation steps, inference heuristics might not be powerful enough to create an index job that produces the correct results. In these cases, explicit index job configuration can be supplied to a repository in two ways (listed below in order of decreasing precedence). Both methods of configuration share a common expected schema. See the [reference documentation](/code_navigation/references/auto_indexing_configuration) for additional information on the shape and content of the configuration.
...
Auto-indexing is currently available for Go, TypeScript, JavaScript, Python, Ruby and JVM repositories. See also [dependency navigation](/code_navigation/explanations/features#dependency-navigation) for instructions on how to setup cross-dependency navigation depending on what language ecosystem you use.
...
The general happy-path for an index job is: `QUEUED_FOR_INDEXING`, `INDEXING`, then `INDEXING_COMPLETED`. Once uploaded, the remaining lifecycle is described in [lifecycle of an upload](/code_navigation/explanations/uploads#lifecycle-of-an-upload).
...
The stdout and stderr of each command run during pre-indexing and indexing steps are viewable as the index job is processed. This information is valuable when troubleshooting a [custom index configuration](/code_navigation/references/auto_indexing_configuration) for your repository.
...
When a commit of a repository is selected as a candidate for [auto-indexing](/code_navigation/explanations/auto_indexing) but does not have an explicitly supplied index job configuration, index jobs are inferred from the content of the repository at that commit.
...
This document describes the heuristics used to determine the set of index jobs to schedule. See [configuration reference](/code_navigation/references/auto_indexing_configuration) for additional documentation on how index jobs are configured.

As a general rule of thumb, an indexer can be invoked successfully if the source code to index can be compiled successfully. The heuristics below attempt to cover the common cases of dependency resolution, but may not be sufficient if the target code requires additional steps such as code generation, header file linking, or installation of system dependencies to compile from a fresh clone of the repository. For such cases, we recommend using the inferred job as a starting point to [explicitly supply index job configuration](/code_navigation/how-to/configure_auto_indexing#explicit-index-job-configuration).
...
<QuickLinks>
	...
	<QuickLink href="/code-search/code-navigation/inference_configuration" icon="lightbulb" imgAlt="Code Navigation" title="Auto-indexing inference configuration reference" description="Guide for site administrators on customizing code intelligence indexing jobs detection in Sourcegraph." />
  <QuickLink href="/code-search/code-navigation/how-to/combining_scip_uploads_from_ci_cd_and_auto_indexing" icon="theming" imgAlt="Code Navigation" title="Combine SCIP uploads from CI/CD & auto-indexing" description="Learn about methods to precise SCIP index data for your team's repositories." />
  <QuickLink href="/code-search/code-navigation/how-to/policies_resource_usage_best_practices" icon="presets" imgAlt="Code Navigation" title="Code intelligence policies resource usage best practices" description="An overview of best practices when defining auto-index scheduling and data retention policies." />
</QuickLinks>
