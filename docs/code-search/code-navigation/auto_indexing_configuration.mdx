...
This document details the expected contents of [explicit index job configuration](/code-search/code-navigation/auto_indexing#explicit-index-job-configuration) for a particular repository. Depending on how this configuration is supplied, it may be encoded as JSON.
...
6. The [`src` CLI](/cli/) binary is invoked to upload the index artifact to the Sourcegraph instance.
...
```json
{
	"indexer": "sourcegraph/scip-go:v0.1@sha256::39c1495...55391cc",
	"indexer_args": ["scip-go", "-q"],
	"root": "dev/sg"
}
```
...
```json
{
	...
	"commands": ["yarn config set ignore-engines true", "yarn install"],
	"root": "lib/proj"
}
```
