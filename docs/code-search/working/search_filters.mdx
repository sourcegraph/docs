...
The search filters UI panel tries to represent most of the filters we have in the search query syntax,
...
## 1. Search filters are built by provided search results

All filters that you can see in the panel are calculated by search matches that Sourcegraph
found based on your query. This means that your query doesn't return any symbols, diff or commit
...
- `type:content` matches within file content
- `type:repo` matches with repositories names
- `type:file` matches with file names
...
**Note:** This is not the desired end state. We're exploring being able to display only code by default (currently `type:content'), but our currently support for `AND` and `OR` operators in the query doesn't sensible allow for this.
...
**Note:** This is not the desired end state. We're working to move the `count` options from the filter panel into the results summary.
...
incorrect query and possibly provide zero match results.
