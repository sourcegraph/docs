...
**Note:** This is not the desired end state. We're exploring being able to display only code by default (currently `type:content'), but our current support for `AND` and `OR` operators in the query doesn't sensibly allow for this.
