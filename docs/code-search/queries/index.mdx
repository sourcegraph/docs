...
<Callout type="tip">Keyword search can be enabled for a Batch Change by adding [patternType:keyword](/code-search/queries/language#pattern-type) to the query in [a batch spec](/batch-changes/quickstart).</Callout>
