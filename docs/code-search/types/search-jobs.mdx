...
If you do hit a timeout, you can try to break down your search query into smaller queries and run them separately.

### A search job failed, how can I find out what went wrong?

We mark a search job as failed if any of the tasks fail.
You can access the log of a search jobs from the search jobs UI.
The log contain one line per task. If a task fails, the log will contain the error message.