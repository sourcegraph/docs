...
You can automate data indexing in SCIP for Go codebases or index data manually.
  ...
  scip-go:
    ...
    steps:
      ...
      - name: Generate SCIP data
        run: scip-go
      - name: Install SG CLI
        run: curl -L https://sourcegraph.com/.api/src-cli/src_linux_amd64 -o /usr/local/bin/src; chmod +x /usr/local/bin/src;
...
1. Upload the data to a Sourcegraph instance with:
   ...
   src -endpoint=<your sourcegraph endpoint> code-intel upload
