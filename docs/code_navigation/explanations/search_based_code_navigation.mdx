...
- Jump to definition: Performs a [symbol search](/code_search/explanations/features#symbol-search).
- Hover documentation: First, finds the definition. Then, extracts documentation from comments near the definition.
- Find references: Performs a case-sensitive word-boundary cross-repository [plain text search](/code_search/explanations/features#powerful-flexible-queries) for the given symbol
