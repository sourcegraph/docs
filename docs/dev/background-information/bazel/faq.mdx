...
It might be because Docker Desktop might not be configured to use the default Docker socket during installation.

Make sure that the checkbox under Docker Desktop -> Settings -> Advanced -> "Allow the default Docker socket to be used (requires password)" is enabled. If it wasn't, then enable it and restart Docker.
