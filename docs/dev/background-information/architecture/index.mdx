...
You can click on each component to jump to its respective code repository or subtree — [Open in a new tab](https://docs.sourcegraph.com/dev/background-information/architecture/architecture.svg)
...
{/* <object
  data="./architecture/architecture.svg"
  type="image/svg+xml"
  width={1023}
  height={1113}
...
If you want to learn more about how repositories are synchronized, read [Life of a repository](/dev/background-information/architecture/life-of-a-repository).
...
If you want to learn more about how repository permissions are synchronized in the background, read about [permission syncing](/admin/permissions/syncing).
...
- [Code search product documentation](/code-search/)
- [Life of a search query](/dev/background-information/architecture/life-of-a-search-query)
- [Indexed ranking](/dev/background-information/architecture/indexed-ranking)
...
By default, Sourcegraph provides imprecise [search-based code navigation](/code_navigation/explanations/search_based_code_navigation). This reuses all the architecture that makes search fast, but it can result in false positives (for example: finding two definitions for a symbol, or references that aren't actually references), or false negatives (for example: not able to find the definition or all references). This is the default because it works with no extra configuration and is pretty good for many use cases and languages. We support a lot of languages this way because it only requires writing a few regular expressions.

With some setup, customers can enable [precise code navigation](/code_navigation/explanations/precise_code_navigation). Repositories add a step to their build pipeline that computes the index for that revision of code and uploads it to Sourcegraph. We have to write language specific indexers, so adding precise code navigation support for new languages is a non-trivial task.

If you want to learn more about code navigation:

- [Code navigation product documentation](/code_navigation/)
- [Code navigation developer documentation](/dev/background-information/codeintel/)
- [Available indexers](/code_navigation/references/indexers)
...
- [Batch Changes product documentation](/batch_changes/)
- [Batch Changes design principles](/batch_changes/explanations/batch_changes_design)
- [Batch Changes developer documentation](/dev/background-information/batch_changes/)
...
Code Insights are persisted in a separate databased called `codeinsights-db`. The web application interacts with the backend through a [GraphQL API](/api/graphql/managing-code-insights-with-api).
...
- [Sourcegraph browser extensions documentation](/integration/browser_extension)
- [Overview of code host integrations](/dev/background-information/web/code_host_integrations)
...
src-cli is written in Go, and distributed as a standalone binary for Windows, macOS, and Linux. Its features include [running searches](/cli/references/search), managing Sourcegraph, and [executing batch changes](/batch_changes/quickstart#create-the-batch-change). src-cli is an integral part of the [batch changes product](#batch-changes).
...
- [Kubernetes](/admin/deploy/kubernetes/) is intended for all medium to large scale production deployments that require fault tolerance and high availibility. For advanced users only with significant kubernetes experience required. This deployment method is developed in [`deploy-sourcegraph`](https://github.com/sourcegraph/deploy-sourcegraph).
...
- [Docker Single Container](/admin/deploy/docker-single-container/) for small environments on a single server. Easiest and quickest to setup with a single command. Little infrastructure knowledge is required. This deployment method is developed in [`cmd/server`](https://sourcegraph.com/github.com/sourcegraph/sourcegraph/-/tree/cmd/server).
...
- [grafana](/dev/background-information/observability/grafana) is the frontend for service metrics, and ships with customized dashboards for Sourcegraph services.
- [prometheus](/dev/background-information/observability/prometheus) handles scraping of service metrics, and ships with recording rules, alert rules, and alerting capabilities.
- [cadvisor](/dev/background-information/observability/cadvisor) provides per-container performance metrics (scraped by Prometheus) in most Sourcegraph environments.
- [Health checks are provided by each Sourcegraph service](/admin/observability/health_checks).
...
- [Logs are provided by each Sourcegraph service](/admin/observability/logs).

If you want to learn more about observability:

- [Observability for site administrators](/admin/observability/)
- [Observability developer documentation](/dev/background-information/observability/)
- [Observability at Sourcegraph](https://handbook.sourcegraph.com/engineering/observability)

## Other resources

- [Life of a ping](/dev/background-information/architecture/life-of-a-ping)
- [Row level security (discontinued)](/dev/background-information/architecture/row_level_security)
