...
- Run `sg wolfi lock gitserver`
  - Inspect the gitserver lockfile at `wolfi-images/gitserver.lock.json` to see that it now include your new package, pinned at a specific version
...
- Run `sg wolfi image gitserver`
  ...
  - As a convenience, `sg wolfi image` will automatically run `sg wolfi lock` whenever the YAML and lockfile are out of sync.
...
- Once happy, commit any changes in `wolfi-images/` and push. Your changes will be present in the Buildkite run.
