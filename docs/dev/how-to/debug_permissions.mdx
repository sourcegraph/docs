...
3. At the bottom of the **Permissions** page you'll find a list of repositories the user has access to and the reason for their access. Possible access reasons are:
    ...
    - **Permissions Sync** - The user has access to this repository because they have connected an external account that has access to this repository on the code host.
    - **Explicit API** - The user has been granted explicit access to this repository by means of the explicit permissions API.
...
2. If a user has not connected their code host account, the user will have to do that by visiting their Account security settings and clicking on Add. Without a connected code host account, Sourcegraph is not able to associate their Sourcegraph identity with a code host identity to map permissions. If you don’t see the provider for your code host here, you have to configure it first.
3. If the account is connected, you can try visiting the Permissions page again and schedule another permissions sync and see if anything changes. If nothing is changing when a permission sync completes, check the outbound requests log to verify that code host API calls are succeeding.
...
3. The Outbound requests page will show a list of all the http requests Sourcegraph is making to outside services. Calls made to do user/repo permissions syncs will show up here as well. You can try to catch a user permission sync “in the act” by scheduling a sync, and then visiting the Outbound requests page.
...
4. Here we can see a bunch of API calls being made to the GitLab API, and they seem to be succeeding. To stop the list from refreshing every 5 seconds, you can click on the Pause updating button, and to see more details you can click on the More info option.
    - You’ll need to use a bit of your own judgment here to figure out if anything looks out of place. Ideally: you want to see calls being made to the code host. You want to see those calls succeed with a 200 OK status code. Look at the URLs to see if they make sense. Does anything seem weird?

## Configure authorization for a code host connection

1. Visit the Site admin settings page and select the Code host connections section. You should see a list of code host connections.
...
![](https://storage.googleapis.com/sourcegraph-assets/Docs/how-to/debug-permissions/gitlab_connection.png)
    ...
    - Not all authorization configurations are the same. Visit the code hosts docs page for more detailed information on how to configure authorization for the specific code host you’re dealing with.
3. Once you’ve made adjustments and saved the config, a sync should be triggered for the code host. If no sync was triggered, hit the Trigger manual sync button.
...
![](https://storage.googleapis.com/sourcegraph-assets/Docs/how-to/debug-permissions/repo_search.png)
3. If the repository does not show up, but you expect it to, the instance may be configured to enforce repository permissions even for site admins. In this case, the site admin will not be able to see the repository unless they have explicit access to the repository. It may be worth temporarily disabling this enforcement for debugging purposes.
...
5. In the site configuration, there will be a line that reads `"authz.enforceForSiteAdmins": true` . Set it to false. Attempt to search for the repository again.
    - If there is no such entry, or if the setting is already set to false, it means that the repository is not being synced to Sourcegraph at all, and the fault is most likely in the code host connection.
