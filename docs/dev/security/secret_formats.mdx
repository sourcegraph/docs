...
The *instance identifier* is intentionally **not** verified when a token is used, so tokens will remain valid if it is modified. For example, the following tokens have the same *token value* so are equivalent:
