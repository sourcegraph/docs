...
- add CLAUDE.md file for claude code `(PR #3957)`
  - N/A
...
- do-init-only flag in `sg release create` `(PR #3767)`
  - NA
- patch release creates git branch for version `(PR #3712)`
  - na
...
- make minor_nightly_cut.sh executable `(PR #4141)`
  - na
...
- add runtype names `(PR #4060)`
  - na

#### Rr

- v2 and v1 next patch return same values `(PR #3956)`
  - na
...
- sub-repo-perms: adjust ipv4 string prefix matcher to use stack instead of heap allocations `(PR #4054)`
  - N/A
...
- unset cxx instead of NoCC `(PR #3958)`
  - na
...
- Nightly Minor Release Pipeline `(PR #3609)`
  - NA
- /latest releaseregistry endpoint returns latest by semver `(PR #3563)`
  - NA
- change developer dialog back to cache first `(PR #3548)`
- nightly release pipeline must test do-init step `(PR #3344)`
  - na
- call bazel directly for generating stitched migration graph in nightly release pipeline `(PR #2487)`
  - NA
