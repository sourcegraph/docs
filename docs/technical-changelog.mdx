...
- dedupliacte some auto-indexing jobs automatically `(PR #7496)`
  - We no longer queue duplicated jobs for repository HEAD auto-indexing.[^1]: Sorry for "stealing" your ticket btw, but I entered this area when debugging the customer issue and at some point I've invested too much time into this to not have any result. I originally thought I could just make a change that populates the policy ID but there was no way of doing this with having one commit associated with exactly one policy. One change led to another. Hence the result.
